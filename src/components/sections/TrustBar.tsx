...
function LogoTile({ label, abbr, niche, win }: Brand) {
  return (
    <div
      className={cn(
        "group/tile relative flex h-[4.75rem] w-[9.25rem] shrink-0 flex-col items-center justify-center overflow-hidden rounded-2xl border border-white/[0.12] bg-gradient-to-b from-white/[0.09] to-[rgba(5,10,28,0.78)] px-3 shadow-[0_12px_40px_rgba(0,0,0,0.38),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl transition-all duration-500 sm:h-[5.1rem] sm:w-[10.5rem]",
    ...
    >
      ...
      />
      {win ? (
        <span className="absolute right-2 top-2 rounded-full border border-cyan-400/35 bg-cyan-500/15 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-cyan-100 opacity-0 transition-all duration-300 group-hover/tile:opacity-100 sm:text-[10px]">
          {win}
        </span>
      ) : null}
      <span
        aria-hidden
        className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500/35 to-violet-500/22 text-xs font-bold tracking-tight text-cyan-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.15)] ring-1 ring-white/15 transition duration-500 group-hover/tile:from-cyan-400/50 group-hover/tile:to-violet-400/30 group-hover/tile:shadow-[0_0_24px_rgba(34,211,238,0.4)]"
      >
        {abbr}
      </span>
      <span className="relative mt-1.5 max-w-[8.5rem] truncate text-center text-[10px] font-semibold uppercase tracking-[0.1em] text-white/62 transition group-hover/tile:text-white/95 sm:text-[11px]">
        {label}
      </span>
      {niche ? (
        <span className="relative mt-0.5 text-[9px] font-medium uppercase tracking-[0.14em] text-white/38 transition group-hover/tile:text-cyan-200/80 sm:text-[10px]">
          {niche}
        </span>
      ) : null}
    </div>
