          ...
          {list.length === 0 ? (
            ...
            list.map((item, i) => (
              ...
              >
                <div
                  className={cn(
                    "relative cursor-pointer overflow-hidden rounded-3xl border border-white/10 bg-dark-800 shadow-xl transition-all duration-500",
                ...
                >
                  <div className="relative aspect-[16/11] w-full overflow-hidden">
                    <video
                      ...
                      playsInline
                      preload="metadata"
                      className="absolute inset-0 h-full w-full scale-[1.06] object-cover transition-transform duration-500 group-hover:scale-100"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent" />
                    <div className="absolute left-4 top-4 max-w-[70%] rounded-2xl bg-black/75 px-3 py-1 text-[10px] font-mono uppercase tracking-wider text-primary/95 sm:text-xs">
                      {t(`filterLabels.${item.nicheKey}` as "filterLabels.medicine")}
                    </div>
                    <div className="absolute right-4 top-4 rounded-2xl bg-black/80 px-4 py-1.5 text-xs font-semibold text-primary shadow-lg sm:text-sm">
                  ...
                  <div className="space-y-3 p-6 sm:p-8">
                    ...
                    <Link
                      href="/kejsy"
                      className={cn(
                        "mt-6 flex w-full items-center justify-center rounded-2xl border border-transparent py-3.5 text-center text-base font-medium text-primary transition-all",
                        "hover:bg-white/5 hover:text-[#7ee8ff] active:scale-[0.98]"
                      )}
                    >
                      {t("gridViewCase")}
                    </Link>
